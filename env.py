...
class SpringBoxEnv(gym.Env):
    ...
    def __init__(self, grid_size, THRESH, CAP = 4):
        ...
        self.pXs = (np.random.rand(self._config['n_part'],2)-.5)*2*self._config['L']
        self.pXs[:self._config['n_part']//2,0] = -np.random.rand(self._config['n_part']//2)*self._config['L']
        self.pXs[self._config['n_part']//2:,0] = +np.random.rand(self._config['n_part']//2)*self._config['L']
    ...
    def step(self, action):
        ...
        for i in range(5):
            self.pXs, self.pVs, self.acc, self.ms, self.fXs, self.fVs = integrate_one_timestep(pXs = self.pXs,
                                                                 pVs = self.pVs,
                                                                 acc = self.acc,
                                                                 ms  = self.ms,
